...
def get_indices(query2idlist, batch_size, num_replicas):
    ...
    for query, idlist in query2idlist.items():
        index, itemid_list, pid_list, query_eng = idlist
...
class CrossDomainEmbDatasetPrompt(data.Dataset):
    def __init__(self, ann_file, len_clip,
        ...
        for i, line in enumerate(open(self.ann_file, 'r')):
            query, itemid_list, pid_list, query_eng = line.strip().split('\t')
            self.train_list.append((query, itemid_list, pid_list, query_eng))
            if itemid_list != 'None':
    ...
    def __getitem__(self, index):
        while True:
            ...
            query_text, itemid_list, pid_list, query_eng = self.train_list[idx]
            query_text = query_text.rstrip()
            query_eng = query_eng.rstrip()
        ...
        if not self.return_img_id:
            return query_text, title, imgs, query_eng
        else:
            return query_text, title, imgs, pid, pid_source, query_eng

    def __len__(self):
        return self.train_num


class CrossDomainDistributedSamplerPrompt(DistributedSampler):
    def __init__(self, dataset, sample_info_file, batch_size, num_replicas=None, rank=None, shuffle=True, seed=0,
                 drop_last=False):
        super(CrossDomainDistributedSamplerPrompt, self).__init__(dataset, num_replicas, rank, shuffle, seed, drop_last)
        ...
        with open(sample_info_file) as fr:
            ...
            for i, line in tqdm(enumerate(lines)):
                ...
                query, itemid_list, pid_list, query_eng = values
                self.query2idlist[query] = [i, itemid_list, pid_list, query_eng]
...
class CollateFnPrompt(object):
    def __init__(self):
        self.tokenizer1 = AutoTokenizer.from_pretrained("hfl/chinese-roberta-wwm-ext")
        self.tokenizer2 = GPT2Tokenizer.from_pretrained("facebook/opt-350m")

    def __call__(self, data):
        batch_data = list(zip(*data))
        # query_text, title_text, imgs, query_eng
        batch_data[0] = self.tokenizer1(list(batch_data[0]), return_tensors='pt', padding=True, truncation=True,
                                        max_length=15)
        batch_data[1] = self.tokenizer1(list(batch_data[1]), return_tensors='pt', padding=True, truncation=True,
                                        max_length=60)
        batch_data[2] = torch.cat(batch_data[2], 0)
        batch_data[-1] = self.tokenizer2(batch_data[-1], truncation=True,  # 超过最大长度截断
